...
#include <string.h>
    ...
    {
        FLAG_NONE = 0,
        FLAG_QUOTE_NONE = 1 /* 1 << 0 */ // 没有引号
        ,
        FLAG_QUOTE_SINGLE = 2 /* 1 << 1 */ // 单引号
        ,
        FLAG_QUOTE_DOUBLE = 4 /* 1 << 2 */ // 双引号

        ,
        FLAG_SQL_ANSI = 8 /* 1 << 3 */ // ANSI SQL
        ,
        FLAG_SQL_MYSQL = 16 /* 1 << 4 */ // MYSQL
    };
...
%immutable;
         ...
         */
        ptr_lookup_fn lookup; // SQL查询回调函数
        void *userdata;

        /*
         *
         */
        int flags; // 各种标志 sqli_flags

        /*
         * pos is the index in the string during tokenization
         */
        size_t pos; // 词法分析中，当前处理位置
         ...
         */
        struct libinjection_sqli_token tokenvec[8]; // token数组，上一个token的类型需要额外的token才能决定
#endif

        /*
         * Pointer to token position in tokenvec, above
         */
        struct libinjection_sqli_token *current; // tokenvec位置索引
         ...
         */
        int stats_comment_ddw; // MYSQL单行注释： '--[whitespace]' or '--[EOF]'的数量
         ...
         *
         */
        int stats_comment_ddx; // 不是mysql单行注释 '-' '-'非空格的数量

        /*
         * c-style comments found  /x .. x/
         */
        int stats_comment_c; // C语言风格注释  /* */

        /* '#' operators or MySQL EOL comments found
         *
         */
        int stats_comment_hash; // # 单行注释

        /*
         * number of tokens folded away
         */
        int stats_folds; // 已经折叠的token的数量

        /*
         * total tokens processed
         */
        int stats_tokens; // 处理的token的数量
    };
