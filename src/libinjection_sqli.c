...
void libinjection_sqli_init(struct libinjection_sqli_state *sf, const char *s, size_t len, int flags)
{
    ...
    sf->s = s;
    sf->slen = len;
    sf->lookup = libinjection_sqli_lookup_word; // 挂载回调函数， 查找指纹库
    sf->userdata = 0;
    sf->flags = flags;
                  ...
                  sf->tokenvec[left + 1].val[1] == 'f'))
        {
            ...
            continue; /* reparse everything, but we probably can advance left, and pos */
                 ...
                 sf->tokenvec[left + 1].type == TYPE_LEFTPARENS && (
                                                                       ...
                                                                       cstrcasecmp("USER_ID", sf->tokenvec[left].val, sf->tokenvec[left].len) == 0 || cstrcasecmp("USER_NAME", sf->tokenvec[left].val, sf->tokenvec[left].len) == 0 ||

                                                                       /* Function in MYSQL */
                                                                       cstrcasecmp("DATABASE", sf->tokenvec[left].val, sf->tokenvec[left].len) == 0 || cstrcasecmp("PASSWORD", sf->tokenvec[left].val, sf->tokenvec[left].len) == 0 || cstrcasecmp("USER", sf->tokenvec[left].val, sf->tokenvec[left].len) == 0 ||
                                                                       ...
                                                                       cstrcasecmp("CURRENT_USER", sf->tokenvec[left].val, sf->tokenvec[left].len) == 0 || cstrcasecmp("CURRENT_DATE", sf->tokenvec[left].val, sf->tokenvec[left].len) == 0 || cstrcasecmp("CURRENT_TIME", sf->tokenvec[left].val, sf->tokenvec[left].len) == 0 || cstrcasecmp("CURRENT_TIMESTAMP", sf->tokenvec[left].val, sf->tokenvec[left].len) == 0 || cstrcasecmp("LOCALTIME", sf->tokenvec[left].val, sf->tokenvec[left].len) == 0 || cstrcasecmp("LOCALTIMESTAMP", sf->tokenvec[left].val, sf->tokenvec[left].len) == 0))
        {
...
int libinjection_sqli_not_whitelist(struct libinjection_sqli_state *sql_state)
{
    ...
    case 3:
    {
        ...
        if (streq(sql_state->fingerprint, "sos") || streq(sql_state->fingerprint, "s&s"))
        {

            if ((sql_state->tokenvec[0].str_open == CHAR_NULL) && (sql_state->tokenvec[2].str_close == CHAR_NULL) && (sql_state->tokenvec[0].str_close == sql_state->tokenvec[2].str_open))
            {
                /*
                 * if ....foo" + "bar....
                 */
